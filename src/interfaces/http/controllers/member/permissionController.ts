...
import { PermissionUseCase } from "@application/usecase/member/permissionUseCase";
...
export class permissionController {
  ...
  static async findById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!id || !companyId) return res.status(400).json({ message: "ID dan companyId wajib ada" });
  ...
  static async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!updatedPermission) return res.status(404).json({ message: "Permission tidak ditemukan" });
  ...
  static async delete(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!deleted) return res.status(404).json({ message: "Permission tidak ditemukan" });
