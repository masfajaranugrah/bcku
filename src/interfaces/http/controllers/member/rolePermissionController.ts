...
import { RolePermissionUseCase } from "@application/usecase/member/rolePermissionUseCase";

const rolePermissionUseCase = new RolePermissionUseCase();
...
static async assignPermissionsToRole(req: Request, res: Response, next: NextFunction) {
  try {
    const { roleId, companyId } = req.params;
  ...
  static async updatePermissionsForRole(req: Request, res: Response, next: NextFunction) {
    try {
      const { roleId, companyId } = req.params;
      const { permissionIds } = req.body;

      if (!roleId || !companyId) return res.status(400).json({ status: "fail", message: "Role ID dan Company ID diperlukan" });
  ...
  static async removeAllPermissionsFromRole(req: Request, res: Response, next: NextFunction) {
    try {
      const { roleId, companyId } = req.params;
