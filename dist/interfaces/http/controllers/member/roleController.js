...
const roleUseCase_1 = require("@application/usecase/member/roleUseCase");
const roleUseCase = new roleUseCase_1.RoleUseCase();
class RoleController {
    ...
    static async findById(req, res, next) {
        try {
            ...
            if (!id)
                return res.status(400).json({ message: "ID role wajib ada" });
            if (!companyId)
                return res.status(400).json({ message: "CompanyId wajib ada" });
